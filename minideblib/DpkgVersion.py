...
class DpkgVersion(object):
    ...
    def __init__(self, ver):
        ...
        if idx != -1:
            ...
            if not valid_epoch.search(str(self.epoch)):
    ...
    def get_without_epoch(self):
        ...
        tmpstr = self.upstream
        if self.revision:
            tmpstr += "-%s" % self.revision
        return tmpstr
...
def strcut(tmpstr, idx, accept):
    """Cut characters from str that are entirely in accept."""
    ret = ""
    while idx < len(tmpstr) and tmpstr[idx] in accept:
        ret += tmpstr[idx]
        idx += 1
    return ret, idx


def deb_order(tmpstr, idx):
    ...
    if idx >= len(tmpstr):
        ...
        return cmp_table.index(tmpstr[idx])
